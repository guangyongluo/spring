...
import com.vilin.spring.chapter14.client.domain.FixedDepositDetails;
...
//		closeFixedDeposit(context.getBean(RestTemplate.class));
	...
	private static void getFixedDepositList(RestTemplate restTemplate) {
		...
		ResponseEntity<List<FixedDepositDetails>> responseEntity = restTemplate.exchange(
	...
	private static void getFixedDeposit(RestTemplate restTemplate) {
		ResponseEntity<FixedDepositDetails> responseEntity = restTemplate
				.getForEntity("http://localhost:8080/spring/fixedDeposits?id=1", FixedDepositDetails.class);
	...
	private static void openInvalidFixedDeposit(RestTemplate restTemplate) {
		...
		ResponseEntity<FixedDepositDetails> responseEntity = restTemplate
				.postForEntity("http://localhost:8080/spring/fixedDeposits", fdd, FixedDepositDetails.class);
	...
	private static void editFixedDeposit(RestTemplate restTemplate) {
		...
		ResponseEntity<FixedDepositDetails> responseEntity = restTemplate
				.exchange("http://localhost:8080/spring/fixedDeposits?id=2",
	...
	private static void closeFixedDeposit(RestTemplate restTemplate) {
		...
		ResponseEntity<String> responseEntity = restTemplate.exchange(
				"http://localhost:8080/spring/fixedDeposits?id=3",
				HttpMethod.DELETE, requestEntity, String.class);
		logger.info("HTTP status code : " + responseEntity.getStatusCode() + ". Response body: " + responseEntity.getBody());
	}
}
